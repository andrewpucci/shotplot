document.addEventListener("DOMContentLoaded", (event) => {
  const rinkURL = { NA: "/shotplot/assets/NHL_Hockey_Rink.svg", IIHF: "/shotplot/assets/IIHF_Rink.svg" };
  ...
  function buildTable(rinkType) {
    ...
    for (var key of Object.keys(tableData)) {
      convertedData.push(convertUnits(tableData[key], rinkType));
    }
  ...
  const setupRink = (rinkSelector) => {
    let rinkType = rinkSelector.options[rinkSelector.selectedIndex].value;
    ...
    ajax.onload = () => {
      ...
      rink.addEventListener("mousedown", (event) => {
        ...
        drawCircle(rink, shotID, shotLocation, rinkType);
      }, false);
    }
  };
